  ...
  <v-dialog v-model="dialog" width="800">
    ...
    <template v-slot:activator="{ on, attrs }">
      <div v-bind="attrs" v-on="on">
        <v-icon color="primary" small> mdi-cash-refund </v-icon>
        Return
      </div>
    </template>
...
export default {
  ...
  methods: {
    ...
    async submit() {
      // Returning an order puts the sold units back into sellable stock.
      // The backend restocks exactly what was deducted for this order's
      // invoice, so only order_id / invoice_id are needed here.
      let payload = {
        order_id: this.item.order_id,
        invoice_id: this.item?.invoice?.id || null,
        return_reason: this.payload.return_reason || null,
      };

      this.loading = true;

      try {
        await this.$axios.post(`return-order`, payload);
        this.close();
        this.$emit("response", "Order has been returned and stock restored.");
      } catch (error) {
